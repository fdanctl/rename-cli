...
var prependCmd = &cobra.Command{
	Use:   "prepend [flags] <prefix> [files...]",
	Args:  cobra.MinimumNArgs(2),
	Short: "Inserts to the begining of filenames with optional numbering and date formatting",
	Long: `Inserts to the begining of filenames with optional numbering and date formatting

Usage Example:
rename-cli prepend -n 1 --dry-run '%YYYY-%MM-%DD' *.jpg
...
`,
	Run: func(cmd *cobra.Command, args []string) {
		...
		for _, file := range files {
			...
			replaced := trailingPath + prefix + filename + extension
...
func init() {
	...
	prependCmd.Flags().IntP("start", "n", 1, "Starting value for %n numbering")
	prependCmd.Flags().Bool("dry-run", false, "Show what would be renamed without making changes")
